...
VertexToPixel CelVertexShader(AppToVertex input)
{
    ...
    output.Position = mul(float4(input.Position.xyz, 1.0), g_WorldViewProj);
 
    // Transform the normal
    output.Normal = mul(float4(input.Normal.xyz, 1.0), (float3x3)g_WorldMatrix);
...
VertexToPixel OutlineVertexShader(AppToVertex input)
{
    ...
    float4 original = mul(float4(input.Position.xyz, 1.0), g_WorldViewProj);
 
    // Calculates the normal of the vertex like it ought to be.
    float4 normal = mul(float4(input.Normal.xyz, 1.0), (float3x3)g_WorldMatrix);
