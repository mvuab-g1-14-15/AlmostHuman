...
{
    CLogger::GetSingletonPtr()->AddNewLog(ELL_INFORMATION, "CFontManager:: Inicializando FontManager");
    ...
    for (int i = 0; i < count; ++i)
    {
            ...
            if (!exist) 
            {
                ...
                if (id==1)
                {
                    ...
                    CLogger::GetSingletonPtr()->AddNewLog(ELL_INFORMATION, "LoadFonts:: Add font %s (file:%s,size:%d,bold:%d,italica:%d,default:%d),",fontId.c_str(), file.c_str(), size, bold, italica, _default); 
                }
                else
                {
                    CLogger::GetSingletonPtr()->AddNewLog(ELL_ERROR, "LoadFonts:: no se ha podido añadir el ttf file: %s", file.c_str()); 
