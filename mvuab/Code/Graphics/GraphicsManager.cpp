...
void CGraphicsManager::Clear()
{
    ...
    mDirectXDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB( red, green, blue ),
                           1.0f, 0 );
...
bool CGraphicsManager::Init( HWND hWnd, CEngineConfig* aEngineConfig )
{
    ...
    {
        LOG_ERROR_APPLICATION( "GraphicsManager::Lack of support for desired adapter mode " );
        return mOk = false;
    }

    LOG_INFO_APPLICATION( "GraphicsManager::Correct suport for the adapter" );
    ...
    if( FAILED( hr = mDirectXObject->CheckDeviceType( D3DADAPTER_DEFAULT,
                     D3DDEVTYPE_HAL,
                     D3DFMT_X8R8G8B8,
                     D3DFMT_X8R8G8B8,
                     FALSE ) ) )
    {
        ...
        LOG_ERROR_APPLICATION( "GraphicsManager::Lack of support for a 32 - bit back buffer" );
        return mOk = false;
    }

    LOG_INFO_APPLICATION( "GraphicsManager::Support for a 32 - bit back buffer" );

    // Can we get a z-buffer that's at least 16 bits?
    if( FAILED( hr = mDirectXObject->CheckDeviceFormat( D3DADAPTER_DEFAULT,
                     D3DDEVTYPE_HAL,
                     D3DFMT_X8R8G8B8,
                     ...
                     D3DFMT_D16 ) ) )
    {
        LOG_ERROR_APPLICATION( "GraphicsManager::Lack of support for a 16 - bit back buffer" );
        return mOk = false;
    }

    LOG_INFO_APPLICATION( "GraphicsManager::Support for a 16 - bit back buffer" );
                ...
                D3DDEVTYPE_HAL, &d3dCaps ) ) )
    {
        ...
        LOG_ERROR_APPLICATION( "GraphicsManager::Failure of GetDeviceCaps" );
        return mOk = false;
    }

    LOG_INFO_APPLICATION( "GraphicsManager::Correct get Device Caps" );
    DWORD flags = 0;
    ...
    ASSERT(aEngineConfig, "No engine configuration");
    d3dpp.Windowed               = (aEngineConfig->GetFullScreenMode()) ? FALSE : TRUE;
    d3dpp.EnableAutoDepthStencil = TRUE;
    d3dpp.AutoDepthStencilFormat = D3DFMT_D16;
    d3dpp.SwapEffect             = D3DSWAPEFFECT_DISCARD;
    d3dpp.BackBufferWidth        = mWidth = aEngineConfig->GetResolutionWidth();
    d3dpp.BackBufferHeight       = mHeight = aEngineConfig->GetResolutionHeight();
    d3dpp.BackBufferFormat       = D3DFMT_X8R8G8B8;
    d3dpp.PresentationInterval   = D3DPRESENT_INTERVAL_IMMEDIATE;

    if( FAILED( mDirectXObject->CreateDevice( D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_WindowId, flags, &d3dpp,
                &mDirectXDevice ) ) )
    {
        // TO DO: Respond to failure of CreateDevice
        LOG_ERROR_APPLICATION( "GraphicsManager::Failure of Creating the device" );
        return mOk = false;
    }

    if( mDirectXDevice == NULL )
    {
        // TO DO: Respond to failure of Direct3DCreate
        FATAL_ERROR("GraphicsManager::The device is Null!!!");
        return mOk = false;
    }

    LOG_INFO_APPLICATION( "GraphicsManager::Correct device initialization" );
    ...
    if( !aEngineConfig->GetFitDesktop())
    {
        LOG_INFO_APPLICATION( "GraphicsManager::Window created in (%d,%d) with size (%d,%d) and resolution(%d,%d)",
                              ...
                              aEngineConfig->GetScreenHeight(), aEngineConfig->GetResolutionWidth(), aEngineConfig->GetResolutionHeight());
