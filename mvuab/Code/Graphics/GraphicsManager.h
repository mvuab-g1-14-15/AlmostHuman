...
public:
  ...
  void DrawIcoSphere();
  void DrawGrid( float32 Size, Math::CColor Color = Math::colWHITE, int GridX = 10,
                 int32 GridZ = 10 );
  ...
  void DrawRectangle2D( const Math::Vect2i& pos, uint32 w, uint32 h, Math::CColor& backGroundColor,
                        uint32 edge_w, uint32 edge_h, Math::CColor& edgeColor );
  void DrawCylinder( float32 Top_Radius, float32 Bottom_Radius, float32 h, uint32 Aristas,
                     Math::CColor Color, bool drawCover );
  void DrawCapsule( float32 radius, float32 h, uint32 Aristas = 10,
                    Math::CColor Color = Math::colWHITE );
  void DrawQuad3D( const Math::Vect3f& pos, const Math::Vect3f& up, const Math::Vect3f& right,
                   float32 w, float32 h, Math::CColor color = Math::colBLUE );
  void DrawQuad3D( const Math::Vect3f& ul, const Math::Vect3f& ur, const Math::Vect3f& dl,
