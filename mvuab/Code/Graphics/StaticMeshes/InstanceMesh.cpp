...
CInstanceMesh::CInstanceMesh(const std::string& Name, const std::string& CoreName) : m_pStaticMesh(CStaticMeshManager::GetSingletonPtr()->GetResource(CoreName)), CRenderableObject()
{
    if (!m_pStaticMesh) LOG_ERROR_APPLICATION("CInstanceMesh::CInstanceMesh No se puede instanciar m_pStaticMesh");
    SetName( Name );
}

CInstanceMesh::CInstanceMesh(CXMLTreeNode &atts) : CRenderableObject(atts), m_pStaticMesh(CStaticMeshManager::GetSingletonPtr()->GetResource(atts.GetPszProperty("core", "no_staticMesh")))
{
}
