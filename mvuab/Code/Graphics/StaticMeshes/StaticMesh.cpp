...
CStaticMesh::CStaticMesh(): m_FileName( "" ),
...
bool CStaticMesh::Load( const std::string& FileName )
{
  ...
  if ( 0 == l_pFile )
  {
    CLogger::GetSingletonPtr()->AddNewLog( ELL_ERROR,
                                           "CStaticMesh::Load No se ha podido abrir \"%s\"!", FileName.c_str() );
    return ( false );
  }
  ...
  {
    CLogger::GetSingletonPtr()->AddNewLog( ELL_ERROR,
                                           "CStaticMesh::Load Header incorrecto!" );
    std::fclose( l_pFile );
    return ( false );
  }
  ...
  for ( unsigned int i = 0; i < l_SubMeshes; ++i )
  {
    ...
    std::fread( l_IdxAddress, sizeof( unsigned short int ) * l_IdxCount, 1,
                l_pFile );
    // Now create the renderable vertex
    CRenderableVertexs* l_RV = NULL;

    if ( l_VertexType == TNORMAL_TAN_BI_T2_DIFF_VERTEX::GetVertexType() )
      l_RV = new CIndexedVertexs<TNORMAL_TAN_BI_T2_DIFF_VERTEX>
      ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
        l_VrtexCount, l_IdxCount );
    else
      if ( l_VertexType == TNORMAL_TAN_BI_T1_DIFF_VERTEX::GetVertexType() )
        l_RV = new CIndexedVertexs<TNORMAL_TAN_BI_T1_DIFF_VERTEX>
        ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
          l_VrtexCount, l_IdxCount );
      else
        if ( l_VertexType == TNORMAL_TAN_BI_T2_VERTEX::GetVertexType() )
          l_RV = new CIndexedVertexs<TNORMAL_TAN_BI_T2_VERTEX>
          ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
            l_VrtexCount, l_IdxCount );
        else
          if ( l_VertexType == TNORMAL_TAN_BI_T1_VERTEX::GetVertexType() )
          {
            CalcTangentsAndBinormals( l_VtxsAddress, ( unsigned short* )l_IdxAddress,
                                      l_VrtexCount, l_IdxCount, l_TypeSize, 0, 12, 24, 36, 48 );
            l_RV = new CIndexedVertexs<TNORMAL_TAN_BI_T1_VERTEX>
            ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
              l_VrtexCount, l_IdxCount );
          }
          else
            if ( l_VertexType == TNORMAL_T2_DIFF_VERTEX::GetVertexType() )
              l_RV = new CIndexedVertexs<TNORMAL_T2_DIFF_VERTEX>
              ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
                l_VrtexCount, l_IdxCount );
            else
              if ( l_VertexType == TNORMAL_T1_DIFF_VERTEX::GetVertexType() )
                l_RV = new CIndexedVertexs<TNORMAL_T1_DIFF_VERTEX>
                ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
                  l_VrtexCount, l_IdxCount );
              else
                if ( l_VertexType == TNORMAL_DIFF_VERTEX::GetVertexType() )
                  l_RV = new CIndexedVertexs<TNORMAL_DIFF_VERTEX>
                  ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
                    l_VrtexCount, l_IdxCount );
                else
                  if ( l_VertexType == TNORMAL_T2_VERTEX::GetVertexType() )
                    l_RV = new CIndexedVertexs<TNORMAL_T2_VERTEX>
                    ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
                      l_VrtexCount, l_IdxCount );
                  else
                    if ( l_VertexType == TNORMAL_T1_VERTEX::GetVertexType() )
                      l_RV = new CIndexedVertexs<TNORMAL_T1_VERTEX>
                      ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
                        l_VrtexCount, l_IdxCount );
                    else
                      if ( l_VertexType == TNORMAL_T1_REFLECTION_VERTEX::GetVertexType() )
                        l_RV = new CIndexedVertexs<TNORMAL_T1_REFLECTION_VERTEX>
                        ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
                          l_VrtexCount, l_IdxCount );
                      else
                        if ( l_VertexType == TT2_DIFF_VERTEX::GetVertexType() )
                          l_RV = new CIndexedVertexs<TT2_DIFF_VERTEX>
                          ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
                            l_VrtexCount, l_IdxCount );
                        else
                          if ( l_VertexType == TT1_DIFF_VERTEX::GetVertexType() )
                            l_RV = new CIndexedVertexs<TT1_DIFF_VERTEX>
                            ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
                              l_VrtexCount, l_IdxCount );
                          else
                            if ( l_VertexType == TDIFF_VERTEX::GetVertexType() )
                              l_RV = new CIndexedVertexs<TDIFF_VERTEX>( CGraphicsManager::GetSingletonPtr(),
                                  l_VtxsAddress, l_IdxAddress, l_VrtexCount, l_IdxCount );
                            else
                              if ( l_VertexType == TT2_VERTEX::GetVertexType() )
                                l_RV = new CIndexedVertexs<TT2_VERTEX>( CGraphicsManager::GetSingletonPtr(),
                                                                        l_VtxsAddress, l_IdxAddress, l_VrtexCount, l_IdxCount );
                              else
                                if ( l_VertexType == TT1_VERTEX::GetVertexType() )
                                  l_RV = new CIndexedVertexs<TT1_VERTEX>( CGraphicsManager::GetSingletonPtr(),
                                                                          l_VtxsAddress, l_IdxAddress, l_VrtexCount, l_IdxCount );
                                else
                                  if ( l_VertexType == TNORMAL_TAN_BI_DIFF_VERTEX::GetVertexType() )
                                    l_RV = new CIndexedVertexs<TNORMAL_TAN_BI_DIFF_VERTEX>
                                    ( CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress,
                                      l_VrtexCount, l_IdxCount );
  ...
  m_AABB = Math::AABB3f( Math::Vect3f( l_AABB[0], l_AABB[1], l_AABB[2] ),
                         Math::Vect3f( l_AABB[3], l_AABB[4], l_AABB[5] ) );
  ...
  {
    CLogger::GetSingletonPtr()->AddNewLog( ELL_ERROR,
                                           "CStaticMesh::Load Footer incorrecto!" );
    std::fclose( l_pFile );
    return ( false );
  }
...
void CStaticMesh::Render( CGraphicsManager* GM )
{
  for ( unsigned int i = 0; i < m_RVs.size(); ++i )
  {
    ...
    std::string l_EffectName =
      l_pEffectManager->GetTechniqueEffectNameByVertexDefault( l_VertexType );
...
bool CStaticMesh::GetRenderableObjectTechnique()
{
  ...
  for ( size_t i = 0; i < m_VertexTypes.size(); ++i )
  {
    if ( m_RenderableObjectTechniqueName == "" )
      m_RenderableObjectTechniqueName =
        l_ROTM->GetRenderableObjectTechniqueNameByVertexType( m_VertexTypes[i] );

    CRenderableObjectTechnique* l_ROT = l_ROTM->GetResource(
                                          m_RenderableObjectTechniqueName );
