...
bool CStaticMesh::Load( const std::string& FileName )
{
    ...
    for (unsigned int i = 0; i < l_SubMeshes; ++i)
    {
        ...
        for (unsigned int j = 0; j < l_numTexturas; ++j)
        {
			...
			std::string lPathTextureName = textureName;
            lPathTextureName = "Data/textures/" + lPathTextureName;
            CTexture* t = TextureMInstance->GetTexture( lPathTextureName );
...
void CStaticMesh::Render( CGraphicsManager* GM )
{
    for(uint32 i = 0; i < m_RVs.size(); ++i)
    {
        ...
        if (i < m_RenderableObjectTechniques.size() && m_RenderableObjectTechniques[i] )
        {
