...
bool CStaticMesh::Load( const std::string& FileName )
{
    ...
    if (0 == l_pFile)
    {
        LOG_ERROR_APPLICATION("CStaticMesh::Load No se ha podido abrir \"%s\"!", FileName.c_str());
        return(false);
    }
    ...
    for (unsigned int i = 0; i < l_SubMeshes; ++i)
    {
        ...
        else if (l_VertexType == TNORMAL_TAN_BI_T1_VERTEX::GetVertexType())
        {
            ...
            l_RV = new CIndexedVertexs<TNORMAL_T1_REFLECTION_VERTEX>(CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress, l_VrtexCount, l_IdxCount);
        else if (l_VertexType == TT2_DIFF_VERTEX::GetVertexType())
        ...
        {
            CalcTangentsAndBinormals(l_VtxsAddress, (unsigned short *)l_IdxAddress, l_VrtexCount, l_IdxCount, l_TypeSize, 0, 12, 28, 44, 60);
            l_RV = new CIndexedVertexs<TRNM_VERTEX>(CGraphicsManager::GetSingletonPtr(), l_VtxsAddress, l_IdxAddress, l_VrtexCount, l_IdxCount);
        }
