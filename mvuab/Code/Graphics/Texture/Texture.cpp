...
bool CTexture::SetAsRenderTarget(size_t IdStage)
{
    ...
    if(FAILED(m_Texture->GetSurfaceLevel(0, &m_RenderTargetTexture))) return false;
    ...
    if(m_CreateDepthStencilSurface)
    {
        if(FAILED(l_Device->GetDepthStencilSurface(&m_OldDepthStencilRenderTarget))) return false;
...
void CTexture::UnsetAsRenderTarget(size_t IdStage, bool UnsetDepthStencilBuffer)
{
    ...
    if(m_CreateDepthStencilSurface)
    {
        if(UnsetDepthStencilBuffer) l_Device->SetDepthStencilSurface(m_OldDepthStencilRenderTarget);
...
bool CTexture::Clone(CTexture** aTexture)
{
    ...
    if(!aTexture) *aTexture = l_AllocatorManger->m_pFreeListAllocator->MakeNew<CTexture>();
