...
CEffectManager::CEffectManager(): CManager()
...
CEffectManager::CEffectManager(CXMLTreeNode &atts) : CManager(atts), mEffectPool(new CEffectPool())
{
}
...
void CEffectManager::CleanUp()
{
    ...
    std::map<std::string, CEffect*> l_Effects = m_Effects.GetResourcesMap();
    CAllocatorManager *l_AllocatorManger = CEngineManagers::GetSingletonPtr()->GetAllocatorManager();

    for(std::map<std::string, CEffect*>::iterator it = l_Effects.begin(); it != l_Effects.end(); it++)
    {
        l_AllocatorManger->m_pFreeListAllocator->MakeDelete(it->second);
        it->second = 0;
    }
        ...
        if(l_TagName == "technique")
        {
            ...
            for(uint32 j = 0, lCurrentNodeCount = l_CurrentNode.GetNumChildren(); j < lCurrentNodeCount; j++)
            {
                ...
                if(l_TagName == "effect")
                {
                    ...
                    l_pEffect = (CEffect *) l_AllocatorManger->m_pFreeListAllocator->Allocate(sizeof(CEffect), __alignof(CEffect));
                    new(l_pEffect) CEffect(l_EffectName);

                    //new CEffect( l_EffectName );
                    ...
                    if(!l_pEffect->Load(l_CurrentSubNode, mEffectPool))
                    {
                        ...
                        l_AllocatorManger->m_pFreeListAllocator->MakeDelete(l_pEffect);
                    }
            ...
            CEffectTechnique* l_NewTechnique = (CEffectTechnique *) l_AllocatorManger->m_pFreeListAllocator->Allocate(sizeof(CEffectTechnique), __alignof(CEffectTechnique));
            new(l_NewTechnique) CEffectTechnique(l_TechniquetName, l_EffectName, l_HandlesNode);
            //new CEffectTechnique( l_TechniquetName, l_EffectName, l_HandlesNode );
            ...
            if(!AddResource(l_TechniquetName, l_NewTechnique))
            {
                ...
                l_AllocatorManger->m_pFreeListAllocator->MakeDelete(l_NewTechnique);
            }
