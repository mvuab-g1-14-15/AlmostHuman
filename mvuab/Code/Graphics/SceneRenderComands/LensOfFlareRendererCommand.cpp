    ...
    CDrawQuadRendererCommand(atts)
{
    ...
    std::string l_TechniqueName = lROTM->GetRenderableObjectTechniqueNameByVertexType(SCREEN_COLOR_VERTEX::GetVertexType());
...
void CLensOfFlareRendererCommand::Execute(CGraphicsManager& GM)
{
    ...
    Math::Vect2f l_ScreenMiddlePoint(float32((width) / 2) + 100 , float32((height) / 2)  + 100);
    ...
    for(int i = 0; i < l_NumOfFlares; ++i)
    {
        ...
        if(l_EffectTech)
        {
            GM.DrawColoredQuad2DTexturedInPixelsByEffectTechnique(l_EffectTech, l_Rect, m_Color, m_StageTextures[i].m_Texture, 0.0f, 0.0f, 1.0f, 1.0f);
        }
        else
        {
            GM.DrawColoredQuad2DTexturedInPixels(l_Rect, m_Color, m_StageTextures[i].m_Texture, 0.0f, 0.0f, 1.0f, 1.0f);
        }
    }
}
...
CLight *CLensOfFlareRendererCommand::GetSun()
{
    ...
    if(l_CurrentCamera->GetFrustum().SphereVisible(D3DXVECTOR3(l_SunPosition.x, l_SunPosition.y, l_SunPosition.z), l_Sun->GetStartRangeAttenuation()))
        return  l_Sun;
