...
{
public:
  CPhysxBone(string _szName) : CName(_szName),m_pCalBone(0), m_iParentID(-1), m_pActor(0), m_bRoot(true),m_pBoneUserData(0), m_pParent(0),m_iCollisionGroup(0) {};
  ~CPhysxBone() { };

  bool                Init(CalBone* _pBone,Math::Mat44f _vMat44,int _iColisionGroup);
  virtual void        Release();

  void                UpdateCal3dFromPhysx();
  void                UpdatePhysxFromCal3d(const Math::Mat44f& _mTransform);

  bool IsBoneRoot(){ return false; }

  //GETERS I SETERS
  CalBone*            GetCalBone()                           {return m_pCalBone;};
  Math::Vect3f        GetMiddlePoint()                       {return m_vMiddlePoint;};
  ...
  const Math::Mat44f& GetTransform()                         {return m_vMatActor;};
  void                SetTransformAfterUpdate(const Math::Mat44f& _mTransform);
  static Math::Mat44f GetBoneLeftHandedAbsoluteTransformation(CalBone* _pBone);
...
private:
  ...
  Math::Mat44f        m_vMatAnimatedModel;
  CalBone*            m_pCalBone;
  int                 m_iParentID;
  Math::Vect3f        m_vMiddlePoint;
  bool                m_bRoot;
  vector<int>         m_vChildListID;
  int                 m_iCollisionGroup;
  Math::Mat44f        m_vMatActor;
