...
void CPhysicRevoluteJoint::SetInfo  (const Math::Vect3f& axis, const Math::Vect3f& anchor, CPhysicActor* actorA,
                                     CPhysicActor* actorB)
{
    if (actorA == NULL)
    {
        LOG_ERROR_APPLICATION( "PhysicRevoluteJoint:: El primer actor pasado como argumento no puede ser null");
        return;
    }
