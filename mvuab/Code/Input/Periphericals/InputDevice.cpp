...
void CInputDevice::Release( void )
{
	...
	CLogger::GetSingletonPtr()->AddNewLog(ELL_INFORMATION, "InputDevice::nput device offline (ok)");
} 
...
HRESULT CInputDevice::CrankUp(REFGUID rguid, LPCDIDATAFORMAT pdf, bool exclusiveMode)
{
	...
	if ( FAILED(m_pDI->CreateDevice(rguid, &m_pDevice, NULL))) {
		CLogger::GetSingletonPtr()->AddNewLog(ELL_ERROR, "InputDevice: CreateDevice failed");
		return S_FALSE; 
	} 

	// set the correct device data format
	if ( FAILED(m_pDevice->SetDataFormat(pdf))) {
		CLogger::GetSingletonPtr()->AddNewLog(ELL_ERROR, "InputDevice: SetDataFormat failed");
		return S_FALSE; 
	}

	// set the cooperation level with windows
	if ( FAILED(m_pDevice->SetCooperativeLevel(m_hWnd, dwFlags))) {
		CLogger::GetSingletonPtr()->AddNewLog(ELL_ERROR, "InputDevice: SetCoopLevel failed");
		return S_FALSE; 
	}
